...
class Enum {
  ...
  _mountEnumObject(obj, options) {
    const enumObject = Object.keys(obj).reduce((acumulator, item) => {
       ...
       * @param {object} customOptions Options to compare
       * @param {boolean} customOptions.ignoreCase Ignore case in comparisons
       * @returns {boolean} The result of the comparison
       */
      acumulator[item].__proto__.is = function(description, customOptions = {}) {
        const ignoreCase = options.ignoreCase || customOptions.ignoreCase
        let value = self.getDescription(this)
